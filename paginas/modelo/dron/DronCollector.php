...
class DronCollector extends collector
{
  ...
  function showDron() {
    ...
    $arrayDron= array();        
    foreach ($rows as $c){
      $aux = new Dron($c{'id_dron'},$c{'marca'},$c{'num_serial'},$c{'id_empresa'},$c{'bateria_minutos'},$c{'costo'});
    ...
    function createDron($marca,$num_serial,$id_empresa,$bateria_minutos) {
     ...
     function updateDron($id,$marca,$num_serial,$id_empresa,$bateria_minutos,$costo) {
        $rows = self::$db->insertRow("UPDATE dron SET marca='$marca' ,num_serial='$num_serial', id_empresa='$id_empresa' ,bateria_minutos= '$bateria_minutos',costo= '$costo' WHERE id_dron='$id'" ,null);
